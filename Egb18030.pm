...
${Egb18030::anchor}      = qr{\G(?:[\x81-\xFE][\x30-\x39][\x81-\xFE][\x30-\x39]|[\x81-\xFE][^\x30-\x39]|[^\x81-\xFE])*?}oxms;
...
if (($] >= 5.010001) or
    ...
    # GB18030 encoding
        ...
        ${Egb18030::q_char_SADAHIRO_Tomoyuki_2002_01_17} = qr{.*?[^\x30-\x39\x81-\xFE](?:[\x30-\x39]|[\x81-\xFE][\x30-\x39][\x81-\xFE][\x30-\x39]|[\x81-\xFE]{2})*?}oxms;
    ...
    # other encoding
        ...
        ${Egb18030::q_char_SADAHIRO_Tomoyuki_2002_01_17} = qr{.*?[$sbcs](?:$tbcs_1st[^$sbcs]{2})*?}oxms;
    }

    ${Egb18030::anchor_SADAHIRO_Tomoyuki_2002_01_17} =
    qr{\G(?(?=.{0,32766}\z)(?:[\x81-\xFE][\x30-\x39][\x81-\xFE][\x30-\x39]|[\x81-\xFE][^\x30-\x39]|[^\x81-\xFE])*?|(?(?=[$sbcs]+\z).*?|(?:${Egb18030::q_char_SADAHIRO_Tomoyuki_2002_01_17})))}oxms;
#   qr{
#      \G # (1), (2)
#        (? # (3)
#          (?=.{0,32766}\z) # (4)
...
sub e_sub {
    ...
    if ($r_modifier) {
        ...
        # s///gr with multibyte anchoring
            ...
            $sub = sprintf(
                #                              1                                              2   3                                 4   5
                q<CORE::eval{local $GB18030::re_t=%s; local $GB18030::re_a=''; while($GB18030::re_t =~ %s){%s local $^W=0; local $GB18030::re_r=%s; %s$GB18030::re_t="$GB18030::re_a${1}$GB18030::re_r$'"; pos($GB18030::re_t)=length "$GB18030::re_a${1}$GB18030::re_r"; $GB18030::re_a=substr($GB18030::re_t,0,pos($GB18030::re_t)); } return $GB18030::re_t}>,
                ...
                $s_matched,                                                      #  3
                $e_replacement,                                                  #  4
                '$GB18030::re_r=CORE::eval $GB18030::re_r; ' x $e_modifier,            #  5
            );
        }
        ...
        # s///gr without multibyte anchoring
            ...
            $sub = sprintf(
                #                              1                        2   3                                 4   5
                q<CORE::eval{local $GB18030::re_t=%s; while($GB18030::re_t =~ %s){%s local $^W=0; local $GB18030::re_r=%s; %s$GB18030::re_t="$`$GB18030::re_r$'"; pos($GB18030::re_t)=length "$`$GB18030::re_r"; } return $GB18030::re_t}>,
        ...
        # s///r
            ...
            $sub = sprintf(
                #  1     2                3                                 4   5  6                    7
                q<(%s =~ %s) ? CORE::eval{%s local $^W=0; local $GB18030::re_r=%s; %s"%s$GB18030::re_r$'" } : %s>,
    ...
    # without /r
        ...
        # s///g with multibyte anchoring
            ...
            $sub = sprintf(
                ...
                q<CORE::eval{local $GB18030::re_n=0; local $GB18030::re_a=''; while(%s =~ %s){%s local $^W=0; local $GB18030::re_r=%s; %s%s="$GB18030::re_a${1}$GB18030::re_r$'"; pos(%s)=length "$GB18030::re_a${1}$GB18030::re_r"; $GB18030::re_a=substr(%s,0,pos(%s)); $GB18030::re_n++} return %s$GB18030::re_n}>,
...
# (and so on)
        ...
        # s///g without multibyte anchoring
            ...
            $sub = sprintf(
                #                                       1     2   3                                 4   5 6                         7                                                 8
                q<CORE::eval{local $GB18030::re_n=0; while(%s =~ %s){%s local $^W=0; local $GB18030::re_r=%s; %s%s="$`$GB18030::re_r$'"; pos(%s)=length "$`$GB18030::re_r"; $GB18030::re_n++} return %s$GB18030::re_n}>,
        ...
        # s///
            ...
            $sub = sprintf(
                ...
                #  1 2 3                4                                 5   6 7   8
                q<(%s%s%s) ? CORE::eval{%s local $^W=0; local $GB18030::re_r=%s; %s%s="%s$GB18030::re_r$'"; 1 } : undef> :

                #  1 2 3                    4                                 5   6 7   8
                q<(%s%s%s) ? 1 : CORE::eval{%s local $^W=0; local $GB18030::re_r=%s; %s%s="%s$GB18030::re_r$'"; undef }>,
                ...
                $s_matched,                                                      #  4
                $e_replacement,                                                  #  5
                '$GB18030::re_r=CORE::eval $GB18030::re_r; ' x $e_modifier,            #  6
                $variable,                                                       #  7
                $prematch,                                                       #  8
            );
        }
