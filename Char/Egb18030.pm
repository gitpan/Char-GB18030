...
BEGIN {
    ...
    if ($^O =~ /\A (?: MSWin32 | NetWare | symbian | dos ) \z/oxms) {
        $PERL5LIB =~ s{[^\x81-\xFE/]*$}{Char::GB18030};
    }
...
if (($] >= 5.010001) or
    ...
    if (__PACKAGE__ =~ / \b Egb18030 \z/oxms) {
        ...
        qr{\G(?(?=.{0,32766}\z)(?:[\x81-\xFE][\x30-\x39][\x81-\xFE][\x30-\x39]|[\x81-\xFE][^\x30-\x39]|[^\x81-\xFE])*?|(?(?=[$sbcs]+\z).*?|(?:.*?[^\x30-\x39\x81-\xFE](?:[\x30-\x39]|[\x81-\xFE][\x30-\x39][\x81-\xFE][\x30-\x39]|[\x81-\xFE]{2})*?)))}oxms;
#       qr{
#          \G # (1), (2)
#            (? # (3)
#              (?=.{0,32766}\z) # (4)
