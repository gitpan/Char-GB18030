...
if ($^O =~ /\A (?: MSWin32 | NetWare | symbian | dos ) \z/oxms) {
    if ($ENV{'ComSpec'} =~ / (?: COMMAND\.COM | CMD\.EXE ) \z /oxmsi) {
        ...
        for (@ARGV) {
            ...
            elsif (m/\A (?:$q_char)*? [*?] /oxms and (my @glob = Char::Egb18030::glob($_))) {
                push @argv, @glob;
            }
            else {
                push @argv, $_;
...
#
# GB18030 fold case lower case without parameter
#
sub Char::Egb18030::fc_() {
    my $s = $_;
    return join '', map {defined($fc{$_}) ? $fc{$_} : $_} ($s =~ m/\G ($q_char) /oxmsg);
...
sub _octets {
    ...
    if ($length == 1) {
        ...
        if (((caller(1))[3] ne 'Char::Egb18030::_octets') and ($modifier =~ m/i/oxms)) {
            if ($a == $z) {
...
sub charlist_not_qr {
    ...
    if (scalar(@charlist) >= 1) {
        if (scalar(@singleoctet) >= 1) {
            ...
            return '(?!' . join('|', @charlist) . ')(?:[\x81-\xFE][\x30-\x39][\x81-\xFE][\x30-\x39]|[\x81-\xFE][\x00-\xFF]|[^\x81-\xFE'. join('', @singleoctet) . '])';
        }
        else {
...
#               $pattern .= "(?:$your_char)",   # UNIX style
        ...
        my $matchsub = sub { Char::Egb18030::fc($_[0]) =~ m{\A $pattern \z}xms };
...
sub _parse_path {
    ...
    my @subpath = ();
    while ($path =~ m{
        ((?: [\x81-\xFE][\x30-\x39][\x81-\xFE][\x30-\x39]|[\x81-\xFE][\x00-\xFF]|[^\x81-\xFE/\\] )+?) [/\\] }oxmsg
    ) {
        push @subpath, $1;
    }
