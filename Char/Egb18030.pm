...
#
{
    ...
    @Char::Egb18030::m_matched = (qr/(?{Char::Egb18030::m_matched})/);
    @Char::Egb18030::s_matched = (qr/(?{Char::Egb18030::s_matched})/);
}
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # POSIX-style character classes
            ...
            $char[$i] = {
                ...
                '[:^lower:]'  => '(?:[\x81-\xFE][\x30-\x39][\x81-\xFE][\x30-\x39]|[\x81-\xFE][\x00-\xFF]|[^\x81-\xFE])',
                '[:^upper:]'  => '(?:[\x81-\xFE][\x30-\x39][\x81-\xFE][\x30-\x39]|[\x81-\xFE][\x00-\xFF]|[^\x81-\xFE])',
...
sub Char::Egb18030::T(;*@) {
    ...
    local $_ = shift if @_;
    croak 'Too many arguments for -T (Char::Egb18030::T)' if @_ and not wantarray;
    my $T = 1;
...
#               $pattern .= "(?:$your_char)",   # UNIX style
            ...
            elsif ((my $uc = Char::Egb18030::uc($char)) ne $char) {
                $pattern .= $uc;
            }
            else {
        ...
        my $matchsub = sub { Char::Egb18030::uc($_[0]) =~ m{\A $pattern \z}xms };
...
=item Split string
  ...
  $string on every match. If the pattern doesn't match at all, Char::Egb18030::split returns
...
=item Chop string
  ...
  The Char::Egb18030::chop operator is used primarily to remove the newline from the end of
  an input record but is more efficient than s/\n$//. If no argument is given, the
  function chops the $_ variable.

=item Index string

  $pos = Char::Egb18030::index($string,$substr,$position);
  $pos = Char::Egb18030::index($string,$substr);
  ...
  the Char::Egb18030::index function returns -1.

=item Reverse index string

  $pos = Char::Egb18030::rindex($string,$substr,$position);
  $pos = Char::Egb18030::rindex($string,$substr);

  Works just like Char::Egb18030::index except that it returns the position of the last
...
=item Make character
  ...
  set. For example, Char::Egb18030::chr(65) is "A" in either ASCII or GB18030, not Unicode,
  and Char::Egb18030::chr(0x82a0) is a GB18030 HIRAGANA LETTER A. For the reverse of
  Char::Egb18030::chr, use Char::GB18030::ord.

=item File test operator -X

  A file test operator is an unary operator that tests a pathname or a filehandle.
  If $string is omitted, it uses $_ by function Char::Egb18030::r_.
  The following functions function when the pathname ends with chr(0x5C) on MSWin32.

  $test = Char::Egb18030::r $string;
  $test = Char::Egb18030::r_;
  ...
  a filehandle. Both Char::Egb18030::T and Char::Egb18030::B deosn't work when given the special
  ...
  $value = Char::Egb18030::s $string;
  $value = Char::Egb18030::s_;
...
=item Statistics about file
  ...
  ($dev,$ino,$mode,$nlink,$uid,$gid,$rdev,$size,
      $atime,$mtime,$ctime,$blksize,$blocks) = Char::Egb18030::stat($file);
...
=item require file
  ...
  If the argument is a string, Char::Egb18030::require loads and executes the Perl code found in
  the separate file whose name is given by the string. This is similar to performing a
  Char::Egb18030::do on a file, except that Char::Egb18030::require checks to see whether the library
  ...
  duplicate compilation.) Like its cousins Char::Egb18030::do and use, Char::Egb18030::require knows how
