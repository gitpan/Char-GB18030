...
sub Char::GB18030::escape_script {
    ...
    if (/^ use Char::Egb18030(?:\s+[0-9\.]*)?\s*; $/oxms) {
        ...
        # use Char::GB18030 version qw(ord reverse);
        $function_ord     = 'ord';
        $function_ord_    = 'ord';
...
# while (<WILDCARD>) --- glob
    ...
    elsif (/\G \b while \s* \( \s* < ((?:[\x81-\xFE][\x30-\x39][\x81-\xFE][\x30-\x39]|[\x81-\xFE][\x00-\xFF]|[^\x81-\xFE>\0\a\e\f\n\r\t])+?) > \s* \) \b /oxgc) {
...
# <WILDCARD> --- glob
    ...
    elsif (/\G < ((?:[\x81-\xFE][\x30-\x39][\x81-\xFE][\x30-\x39]|[\x81-\xFE][\x00-\xFF]|[^\x81-\xFE>\0\a\e\f\n\r\t])+?) > /oxgc) {
...
=head1 SYNOPSIS
  ...
  use Char::GB18030 qw(ord reverse); --- demand enhanced feature of ord and reverse
  use Char::GB18030 ver.sion qw(ord reverse);
  use Char::GB18030 ver.sion.0 qw(ord reverse);
