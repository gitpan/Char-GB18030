...
# while (<WILDCARD>) --- glob
    ...
    elsif (/\G \b while \s* \( \s* < ((?:[\x81-\xFE][\x30-\x39][\x81-\xFE][\x30-\x39]|[\x81-\xFE][\x00-\xFF]|[^>\0\a\e\f\n\r\t])+?) > \s* \) \b /oxgc) {
...
# <WILDCARD> --- glob
    ...
    elsif (/\G < ((?:[\x81-\xFE][\x30-\x39][\x81-\xFE][\x30-\x39]|[\x81-\xFE][\x00-\xFF]|[^>\0\a\e\f\n\r\t])+?) > /oxgc) {
...
sub classic_character_class {
    ...
    return {
        '.'  => ($modifier =~ /s/) ?
                '(?:[\x81-\xFE][\x30-\x39][\x81-\xFE][\x30-\x39]|[\x81-\xFE][\x00-\xFF]|[\x00-\xFF])' :
                '(?:[\x81-\xFE][\x30-\x39][\x81-\xFE][\x30-\x39]|[\x81-\xFE][\x00-\xFF]|[^\x0A])',
...
sub e_use_noimport {
    ...
    for my $realfilename (_realfilename($expr)) {
        ...
        if (CORE::open($fh, $realfilename)) {
            ...
            if ($script =~ m/^ \s* use \s+ Char::GB18030 \s* ([^;]*) ; \s* \n? $/oxms) {
...
and the cellular phone. This software treats GB18030 directly. Therefor there is
not UTF8 flag.
...
The character classes are redefined as follows to backward compatibility.
  ...
  ---------------------------------------------------------------------------
   .          (?:[\x81-\xFE][\x30-\x39][\x81-\xFE][\x30-\x39]|[\x81-\xFE][\x00-\xFF]|[^\x0A])
              (?:[\x81-\xFE][\x30-\x39][\x81-\xFE][\x30-\x39]|[\x81-\xFE][\x00-\xFF]|[\x00-\xFF]) (/s modifier)
  \d          [0-9]
  \s          [\x09\x0A\x0C\x0D\x20]
  \w          [0-9A-Z_a-z]
  \D          (?:[\x81-\xFE][\x30-\x39][\x81-\xFE][\x30-\x39]|[\x81-\xFE][\x00-\xFF]|[^0-9])
...
=head1 Perl5.6 Emulation On perl5.005
  ...
  --------------------------------------------------------------------
  Before          After                  in BEGIN { } of Char/Egb18030.pm
  --------------------------------------------------------------------
  binmode(...);   Char::Egb18030::binmode(...);   *CORE::GLOBAL::binmode = ...
  open(...);      Char::Egb18030::open(...);      *CORE::GLOBAL::open    = ...
  --------------------------------------------------------------------
...
=item * binmode (Perl5.6 emulation on perl5.005)
  ...
  * two arguments
  ...
  If you are using perl5.005 other than MacPerl, Char::GB18030 software emulate perl5.6's
...
=item * open (Perl5.6 emulation on perl5.005)
  ...
  * three arguments
  ...
  If you are using perl5.005, Char::GB18030 software emulate perl5.6's open function.
  ...
  open(FH, "<:crlf:encoding(Char::GB18030)", $path) or die "can't open $path: $!";
