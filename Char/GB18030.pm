...
######################################################################
#
# Char::GB18030 - Source code filter to escape GB18030 script
#
# http://search.cpan.org/dist/Char-Char::GB18030/
#
...
BEGIN { CORE::require Char::Egb18030; }
...
sub unimport {}
sub Char::GB18030::escape_script;
...
my $function_ord;         # ord()   to ord()   or Char::GB18030::ord()
my $function_ord_;        # ord     to ord     or Char::GB18030::ord_
my $function_reverse;     # reverse to reverse or Char::GB18030::reverse
my $function_getc;        # getc    to getc    or Char::GB18030::getc
...
END
    ...
    print Char::GB18030::escape_script($ARGV[0]);
    exit 0;
}
...
sub import {
    ...
    if (Char::Egb18030::e("$filename.e")) {
        if (exists $ENV{'SJIS_DEBUG'}) {
            Char::Egb18030::unlink "$filename.e";
        }
        elsif (Char::Egb18030::z("$filename.e")) {
            Char::Egb18030::unlink "$filename.e";
        }
        else {
            my $e_mtime   = (Char::Egb18030::stat("$filename.e"))[9];
            my $mtime     = (Char::Egb18030::stat($filename))[9];
            my $__mtime__ = (Char::Egb18030::stat(__FILE__))[9];
    ...
    if (not Char::Egb18030::e("$filename.e")) {
        my $fh = gensym();

        if (eval q{ use Fcntl qw(O_WRONLY O_APPEND O_CREAT); 1 } and CORE::sysopen($fh,"$filename.e",&O_WRONLY|&O_APPEND|&O_CREAT)) {
        }
        else {
            Char::Egb18030::_open_a($fh, "$filename.e") or die __FILE__, ": Can't write open file: $filename.e";
        }
        ...
        my $e_script = Char::GB18030::escape_script($filename);
        print {$fh} $e_script;

        my $mode = (Char::Egb18030::stat($filename))[2] & 0777;
    ...
    my $fh = gensym();
    Char::Egb18030::_open_r($fh, "$filename.e") or die __FILE__, ": Can't read open file: $filename.e";
...
sub Char::GB18030::escape_script {
    ...
    # read GB18030 script
    my $fh = gensym();
    Char::Egb18030::_open_r($fh, $script) or die __FILE__, ": Can't open file: $script";
    local $/ = undef; # slurp mode
    $_ = <$fh>;
    close($fh) or die __FILE__, ": Can't close file: $script";

    if (/^ use Char::Egb18030(?:\s+[0-9\.]*)?\s*; $/oxms) {
        return $_;
    }
    else {
        ...
        $e_script .= sprintf("use Char::Egb18030 %s;\n", $Char::GB18030::VERSION); # require run-time routines version

        # use Char::GB18030 version qw(ord reverse getc);
        $function_ord     = 'ord';
        $function_ord_    = 'ord';
        ...
        if (s/^ \s* use \s+ Char::GB18030 \s* ([^\x81-\xFE;]*) ; \s* \n? $//oxms) {
            ...
            if ($list =~ s/\A ([0-9]+\.[0-9]+) \.0 \s* //oxms) {
                my $version = $1;
                if ($version ne $Char::GB18030::VERSION) {
                    my @file = grep -e, map {qq{$_/Char/GB18030.pm}} @INC;
...
END
                    ...
                    die "Script $0 expects Char/GB18030.pm $version, but @{[__FILE__]} is version $Char::GB18030::VERSION\n";
                }
                $e_script .= qq{die "Script \$0 expects Char/Egb18030.pm $version, but \\\$Char::Egb18030::VERSION is \$Char::Egb18030::VERSION" if \$Char::Egb18030::VERSION ne '$version';\n};
            }
            elsif ($list =~ s/\A ([0-9]+(?:\.[0-9]*)) \s* //oxms) {
                my $version = $1;
                if ($version > $Char::GB18030::VERSION) {
                    die "Script $0 required Char/GB18030.pm $version, but @{[__FILE__]} is only version $Char::GB18030::VERSION\n";
                }
            }
            ...
            if ($list !~ /\A \s* \z/oxms) {
                ...
                my @list = eval $list;
                for (@list) {
                    $function_ord     = 'Char::GB18030::ord'     if /\A ord \z/oxms;
                    $function_ord_    = 'Char::GB18030::ord_'    if /\A ord \z/oxms;
                    $function_reverse = 'Char::GB18030::reverse' if /\A reverse \z/oxms;
                    $function_getc    = 'Char::GB18030::getc'    if /\A getc \z/oxms;
                }
            }
        }
    }
...
# functions of package Char::Egb18030
    ...
    elsif (/\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) qq \s* (\S) ((?:$qq_char)+?)             (\3) /oxgc) { $slash = 'm//'; return "Char::Egb18030::filetest(qw($1)," . e_qq('qq',$2,$4,$3) . ")"; }

    elsif (/\G (-[rwxoRWXOezfdlpSbcugkTB](?:\s+-[rwxoRWXOezfdlpSbcugkTB])+)
                                                         \s* (\') ((?:\\\1|\\\\|$q_char)+?)    (\') /oxgc) { $slash = 'm//'; return "Char::Egb18030::filetest(qw($1)," . e_q ('',  $2,$4,$3) . ")"; }
    ...
    elsif (/\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) q  \s* (\S) ((?:\\\1|\\\\|$q_char)+?)    (\3) /oxgc) { $slash = 'm//'; return "Char::Egb18030::filetest(qw($1)," . e_q ('q', $2,$4,$3) . ")"; }
    ...
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ qq \s* (\S) ((?:$qq_char)+?)             (\3) /oxgc)    { $slash = 'm//'; return "Char::Egb18030::$1(" . e_qq('qq',$2,$4,$3) . ")"; }

    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+    \s* (\') ((?:\\\1|\\\\|$q_char)+?)    (\') /oxgc)    { $slash = 'm//'; return "Char::Egb18030::$1(" . e_q ('',  $2,$4,$3) . ")"; }
    ...
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ q  \s* (\S) ((?:\\\1|\\\\|$q_char)+?)    (\3) /oxgc)    { $slash = 'm//'; return "Char::Egb18030::$1(" . e_q ('q', $2,$4,$3) . ")"; }
...
# functions of package Char::Egb18030
        ...
        elsif ($string =~ /\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) qq \s* (\S) ((?:$qq_char)+?)             (\3) /oxgc) { $e_string .= "Char::Egb18030::filetest(qw($1)," . e_qq('qq',$2,$4,$3) . ")"; $slash = 'm//'; }

        elsif ($string =~ /\G (-[rwxoRWXOezfdlpSbcugkTB](?:\s+-[rwxoRWXOezfdlpSbcugkTB])+)
                                                                        \s* (\') ((?:\\\1|\\\\|$q_char)+?)    (\') /oxgc) { $e_string .= "Char::Egb18030::filetest(qw($1)," . e_q ('',  $2,$4,$3) . ")"; $slash = 'm//'; }
        ...
        elsif ($string =~ /\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) q  \s* (\S) ((?:\\\1|\\\\|$q_char)+?)    (\3) /oxgc) { $e_string .= "Char::Egb18030::filetest(qw($1)," . e_q ('q', $2,$4,$3) . ")"; $slash = 'm//'; }
        ...
        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ qq \s* (\S) ((?:$qq_char)+?)             (\3) /oxgc)    { $e_string .= "Char::Egb18030::$1(" . e_qq('qq',$2,$4,$3) . ")"; $slash = 'm//'; }

        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+    \s* (\') ((?:\\\1|\\\\|$q_char)+?)    (\') /oxgc)    { $e_string .= "Char::Egb18030::$1(" . e_q ('',  $2,$4,$3) . ")"; $slash = 'm//'; }
        ...
        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ q  \s* (\S) ((?:\\\1|\\\\|$q_char)+?)    (\3) /oxgc)    { $e_string .= "Char::Egb18030::$1(" . e_q ('q', $2,$4,$3) . ")"; $slash = 'm//'; }
...
sub e_sub {
    ...
    if ($r_modifier) {
        ...
        # s///gr with multibyte anchoring
            ...
            $sub = sprintf(
                #                        1                                              2   3                                 4   5
                q<eval{local $Char::GB18030::re_t=%s; local $Char::GB18030::re_a=''; while($Char::GB18030::re_t =~ %s){%s local $^W=0; local $Char::GB18030::re_r=%s; %s$Char::GB18030::re_t="$Char::GB18030::re_a${1}$Char::GB18030::re_r$'"; pos($Char::GB18030::re_t)=length "$Char::GB18030::re_a${1}$Char::GB18030::re_r"; $Char::GB18030::re_a=substr($Char::GB18030::re_t,0,pos($Char::GB18030::re_t)); } return $Char::GB18030::re_t}>,
                ...
                $s_matched,                                                      #  3
                $e_replacement,                                                  #  4
                '$Char::GB18030::re_r=eval $Char::GB18030::re_r; ' x $e_modifier,                  #  5
            );
        }
        ...
        # s///gr without multibyte anchoring
            ...
            $sub = sprintf(
                #                        1                        2   3                                 4   5
                q<eval{local $Char::GB18030::re_t=%s; while($Char::GB18030::re_t =~ %s){%s local $^W=0; local $Char::GB18030::re_r=%s; %s$Char::GB18030::re_t="$`$Char::GB18030::re_r$'"; pos($Char::GB18030::re_t)=length "$`$Char::GB18030::re_r"; } return $Char::GB18030::re_t}>,
        ...
        # s///r
            ...
            $sub = sprintf(
                #  1     2          3                                 4   5  6                    7
                q<(%s =~ %s) ? eval{%s local $^W=0; local $Char::GB18030::re_r=%s; %s"%s$Char::GB18030::re_r$'" } : %s>,
    ...
    # without /r
        ...
        # s///g with multibyte anchoring
            ...
            $sub = sprintf(
                ...
                q<eval{local $Char::GB18030::re_n=0; local $Char::GB18030::re_a=''; while(%s =~ %s){%s local $^W=0; local $Char::GB18030::re_r=%s; %s%s="$Char::GB18030::re_a${1}$Char::GB18030::re_r$'"; pos(%s)=length "$Char::GB18030::re_a${1}$Char::GB18030::re_r"; $Char::GB18030::re_a=substr(%s,0,pos(%s)); $Char::GB18030::re_n++} return %s$Char::GB18030::re_n}>,
...
# (and so on)
        ...
        # s///g without multibyte anchoring
            ...
            $sub = sprintf(
                #                                 1     2   3                                 4   5 6                         7                                                 8
                q<eval{local $Char::GB18030::re_n=0; while(%s =~ %s){%s local $^W=0; local $Char::GB18030::re_r=%s; %s%s="$`$Char::GB18030::re_r$'"; pos(%s)=length "$`$Char::GB18030::re_r"; $Char::GB18030::re_n++} return %s$Char::GB18030::re_n}>,
        ...
        # s///
            ...
            $sub = sprintf(
                ...
                #  1 2 3          4                                 5   6 7   8
                q<(%s%s%s) ? eval{%s local $^W=0; local $Char::GB18030::re_r=%s; %s%s="%s$Char::GB18030::re_r$'"; 1 } : undef> :

                #  1 2 3              4                                 5   6 7   8
                q<(%s%s%s) ? 1 : eval{%s local $^W=0; local $Char::GB18030::re_r=%s; %s%s="%s$Char::GB18030::re_r$'"; undef }>,
                ...
                $s_matched,                                                      #  4
                $e_replacement,                                                  #  5
                '$Char::GB18030::re_r=eval $Char::GB18030::re_r; ' x $e_modifier,                  #  6
                $variable,                                                       #  7
                $prematch,                                                       #  8
            );
        }
...
=item * You'll have to download Char/GB18030.pm and Char/Egb18030.pm and put it in your perl lib directory.

=item * You'll need to write "use Char::GB18030;" at head of the script.
...
=head1 SYNOPSIS
  ...
  use Char::GB18030;
  use Char::GB18030 ver.sion;             --- require minimum version
  use Char::GB18030 ver.sion.0;           --- expects version (match or die)
  use Char::GB18030 qw(ord reverse getc); --- demand enhanced feature of ord, reverse, and getc
  use Char::GB18030 ver.sion qw(ord reverse getc);
  use Char::GB18030 ver.sion.0 qw(ord reverse getc);

  # "no Char::GB18030;" not supported

  or

  $ perl Char/GB18030.pm GB18030_script.pl > Escaped_script.pl.e
...
Char::GB18030 software is "middleware" between perl interpreter and your Perl script
written in GB18030.
...
By "use Char::GB18030;", it automatically interpret your script as GB18030. The various
...
=head1 JRE: JPerl Runtime Environment
  ...
  +---------------------------------------+
  |  Source Code Filter, Runtime Routine  | ex. Char/GB18030.pm, Char/Egb18030.pm
  +---------------------------------------+
...
expand wildcard arguments supplied to programs. Instead, import of Char/Egb18030.pm
works well.
...
=head1 Software Composition
   ...
   Char/GB18030.pm               --- source code filter to escape GB18030
   Char/Egb18030.pm              --- run-time routines for Char/GB18030.pm
...
You need write 'use Char::GB18030;' in your script.

  ---------------------------------
  Before      You do
  ---------------------------------
  (nothing)   use Char::GB18030;
  ---------------------------------

=head1 Escaping Multiple-Octet Code (Char/GB18030.pm provides)
...
=head1 Multiple-Octet Anchoring of Regular Expression (Char/GB18030.pm provides)

Char/GB18030.pm applies multiple-octet anchoring at beginning of regular expression.

  --------------------------------------------------------------------------------
  Before                  After
  --------------------------------------------------------------------------------
  m/regexp/               m/${Char::Egb18030::anchor}(?:regexp).../
  --------------------------------------------------------------------------------

=head1 Escaping Second Octet (Char/GB18030.pm provides)

Char/GB18030.pm escapes second octet of multiple-octet character in regular expression.
...
=head1 Multiple-Octet Character Regular Expression (Char/GB18030.pm provides)

Char/GB18030.pm clusters multiple-octet character with quantifier, makes cluster from
...
from classic Perl character class shortcuts and POSIX-style character classes.
  ...
  m/...\D.../             m/...(?:${Char::Egb18030::eD}).../
  m/...[[:^digit:]].../   m/...(?:${Char::Egb18030::not_digit}).../
  --------------------------------------------------------------------------------

=head1 Calling 'Char::Egb18030::ignorecase()' (Char/GB18030.pm provides)

Char/GB18030.pm applies calling 'Char::Egb18030::ignorecase()' instead of /i modifier.

  --------------------------------------------------------------------------------
  Before                  After
  --------------------------------------------------------------------------------
  m/...$var.../i          m/...@{[Char::Egb18030::ignorecase($var)]}.../
  --------------------------------------------------------------------------------
...
Regular expression works as character-oriented that has no /b modifier.
  ...
  Before                  After
  --------------------------------------------------------------------------------
  /regexp/                /ditto$Char::Egb18030::matched/
  m/regexp/               m/ditto$Char::Egb18030::matched/
  ?regexp?                m?ditto$Char::Egb18030::matched?
  m?regexp?               m?ditto$Char::Egb18030::matched?
 
  $_ =~                   ($_ =~ m/ditto$Char::Egb18030::matched/) ?
  s/regexp/replacement/   eval{ Char::Egb18030::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; 1 } :
                          undef
 
  $_ !~                   ($_ !~ m/ditto$Char::Egb18030::matched/) ?
  s/regexp/replacement/   1 :
                          eval{ Char::Egb18030::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; undef }
 
  split(/regexp/)         Char::Egb18030::split(qr/regexp/)
  split(m/regexp/)        Char::Egb18030::split(qr/regexp/)
  split(qr/regexp/)       Char::Egb18030::split(qr/regexp/)
  qr/regexp/              qr/ditto$Char::Egb18030::matched/
  --------------------------------------------------------------------------------
...
Regular expression works as byte-oriented that has /b modifier.
  ...
  Before                  After
  --------------------------------------------------------------------------------
  /regexp/b               /(?:regexp)$Char::Egb18030::matched/
  m/regexp/b              m/(?:regexp)$Char::Egb18030::matched/
  ?regexp?b               m?regexp$Char::Egb18030::matched?
  m?regexp?b              m?regexp$Char::Egb18030::matched?
 
  $_ =~                   ($_ =~ m/(\G[\x00-\xFF]*?)(?:regexp)$Char::Egb18030::matched/) ?
  s/regexp/replacement/b  eval{ Char::Egb18030::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; 1 } :
                          undef
  ...
  qr/regexp/b             qr/(?:regexp)$Char::Egb18030::matched/
  --------------------------------------------------------------------------------

=head1 Escaping Character Classes (Char/Egb18030.pm provides)
...
The character classes are redefined as follows to backward compatibility.
  ...
  ---------------------------------------------------------------
   .            ${Char::Egb18030::dot}
                ${Char::Egb18030::dot_s}    (/s modifier)
  \d            [0-9]
  \s            \s
  \w            [0-9A-Z_a-z]
  \D            ${Char::Egb18030::eD}
...
Definitions in Char/Egb18030.pm.
...
=head1 Un-Escaping \ of \N, \p, \P and \X (Char/GB18030.pm provides)

Char/GB18030.pm removes '\' at head of alphanumeric regexp metasymbols \N, \p, \P
...
=head1 Escaping Built-in Functions (Char/GB18030.pm and Char/Egb18030.pm provide)

Insert 'Char::Egb18030::' at head of function name. Char/Egb18030.pm provides your script Char::Egb18030::*
functions.
  ...
  substr      substr           Byte
  pos         pos              Byte
  split       Char::Egb18030::split     Character
  tr///       Char::Egb18030::tr        Character
  tr///b      tr///            Byte
  tr///B      tr///            Byte
  y///        Char::Egb18030::tr        Character
  y///b       tr///            Byte
  y///B       tr///            Byte
  chop        Char::Egb18030::chop      Character
  index       Char::Egb18030::index     Character
  rindex      Char::Egb18030::rindex    Character
  lc          Char::Egb18030::lc        Character
  lcfirst     Char::Egb18030::lcfirst   Character
  uc          Char::Egb18030::uc        Character
  ucfirst     Char::Egb18030::ucfirst   Character
  fc          Char::Egb18030::fc        Character
  chr         Char::Egb18030::chr       Character
  glob        Char::Egb18030::glob      Character
  lstat       Char::Egb18030::lstat     Character
  opendir     Char::Egb18030::opendir   Character
  stat        Char::Egb18030::stat      Character
  unlink      Char::Egb18030::unlink    Character
  chdir       Char::Egb18030::chdir     Character
  do          Char::Egb18030::do        Character
  require     Char::Egb18030::require   Character
  -------------------------------------------

  ------------------------------------------------------------------------------------------------------------------------
  Before                   After
  ------------------------------------------------------------------------------------------------------------------------
  use Perl::Module;        BEGIN { Char::Egb18030::require 'Perl/Module.pm'; Perl::Module->import() if Perl::Module->can('import'); }
  use Perl::Module @list;  BEGIN { Char::Egb18030::require 'Perl/Module.pm'; Perl::Module->import(@list) if Perl::Module->can('import'); }
  use Perl::Module ();     BEGIN { Char::Egb18030::require 'Perl/Module.pm'; }
  no Perl::Module;         BEGIN { Char::Egb18030::require 'Perl/Module.pm'; Perl::Module->unimport() if Perl::Module->can('unimport'); }
  no Perl::Module @list;   BEGIN { Char::Egb18030::require 'Perl/Module.pm'; Perl::Module->unimport(@list) if Perl::Module->can('unimport'); }
  no Perl::Module ();      BEGIN { Char::Egb18030::require 'Perl/Module.pm'; }
  ------------------------------------------------------------------------------------------------------------------------

=head1 Escaping File Test Operators (Char/GB18030.pm and Char/Egb18030.pm provide)

Insert 'Char::Egb18030::' instead of '-' of operator.

  Available in MSWin32, MacOS, and UNIX-like systems
  --------------------------------------------------------------------------
  Before   After      Meaning
  --------------------------------------------------------------------------
  -r       Char::Egb18030::r   File or directory is readable by this (effective) user or group
  -w       Char::Egb18030::w   File or directory is writable by this (effective) user or group
  -e       Char::Egb18030::e   File or directory name exists
  -x       Char::Egb18030::x   File or directory is executable by this (effective) user or group
  -z       Char::Egb18030::z   File exists and has zero size (always false for directories)
  -f       Char::Egb18030::f   Entry is a plain file
  -d       Char::Egb18030::d   Entry is a directory
  -t       -t         The filehandle is a TTY (as reported by the isatty() system function;
                      filenames can't be tested by this test)
  -T       Char::Egb18030::T   File looks like a "text" file
  -B       Char::Egb18030::B   File looks like a "binary" file
  -M       Char::Egb18030::M   Modification age (measured in days)
  -A       Char::Egb18030::A   Access age (measured in days)
  -C       Char::Egb18030::C   Inode-modification age (measured in days)
  -s       Char::Egb18030::s   File or directory exists and has nonzero size
  ...
  Before   After      Meaning
  --------------------------------------------------------------------------
  -R       Char::Egb18030::R   File or directory is readable by this real user or group
  -W       Char::Egb18030::W   File or directory is writable by this real user or group
  -X       Char::Egb18030::X   File or directory is executable by this real user or group
  -l       Char::Egb18030::l   Entry is a symbolic link
  -S       Char::Egb18030::S   Entry is a socket
  --------------------------------------------------------------------------
  ...
  Before   After      Meaning
  --------------------------------------------------------------------------
  -o       Char::Egb18030::o   File or directory is owned by this (effective) user
  -O       Char::Egb18030::O   File or directory is owned by this real user
  -p       Char::Egb18030::p   Entry is a named pipe (a "fifo")
  -b       Char::Egb18030::b   Entry is a block-special file (like a mountable disk)
  -c       Char::Egb18030::c   Entry is a character-special file (like an I/O device)
  -u       Char::Egb18030::u   File or directory is setuid
  -g       Char::Egb18030::g   File or directory is setgid
  -k       Char::Egb18030::k   File or directory has the sticky bit set
  --------------------------------------------------------------------------
...
You need write 'Char::GB18030::' at head of function name when you want character-
oriented function. See 'Character-Oriented Functions'.

  --------------------------------------------------------
  Function   Character-Oriented   Description
  --------------------------------------------------------
  ord        Char::GB18030::ord
  reverse    Char::GB18030::reverse
  getc       Char::GB18030::getc
  length     Char::GB18030::length
  substr     Char::GB18030::substr
  index      Char::GB18030::index          See 'About Indexes'
  rindex     Char::GB18030::rindex         See 'About Rindexes'
  --------------------------------------------------------
  ...
  (same as Char::Egb18030::index)
  Char::GB18030::index    Character   Character    Character-oriented semantics
  ...
  (same as Char::Egb18030::rindex)
  Char::GB18030::rindex   Character   Character    Character-oriented semantics
...
=item * Ordinal Value of Character
  ...
  $ord = Char::GB18030::ord($string);

  This function returns the numeric value (ASCII or GB18030 character) of the
  first character of $string, not Unicode. If $string is omitted, it uses $_.
  The return value is always unsigned.

  If you import ord "use Char::GB18030 qw(ord);", ord of your script will be rewritten in
  Char::GB18030::ord. Char::GB18030::ord is not compatible with ord of JPerl.

=item * Reverse List or String

  @reverse = Char::GB18030::reverse(@list);
  $reverse = Char::GB18030::reverse(@list);
  ...
  If you import reverse "use Char::GB18030 qw(reverse);", reverse of your script will be
  rewritten in Char::GB18030::reverse. Char::GB18030::reverse is not compatible with reverse of
  JPerl.
...
=item * Returns Next Character
  ...
  $getc = Char::GB18030::getc(FILEHANDLE);
  $getc = Char::GB18030::getc($filehandle);
  $getc = Char::GB18030::getc;
  ...
  If you import getc "use Char::GB18030 qw(getc);", getc of your script will be rewritten in
  Char::GB18030::getc. Char::GB18030::getc is not compatible with getc of JPerl.

=item * Length by GB18030 Character

  $length = Char::GB18030::length($string);
  $length = Char::GB18030::length();

  This function returns the length in characters (programmer-visible characters) of
  the scalar value $string. If $string is omitted, it returns the Char::GB18030::length of
  $_.

  Do not try to use Char::GB18030::length to find the size of an array or hash. Use scalar
...
=item * Substr by GB18030 Character
  ...
  $substr = Char::GB18030::substr($string,$offset,$length,$replacement);
  $substr = Char::GB18030::substr($string,$offset,$length);
  $substr = Char::GB18030::substr($string,$offset);
  ...
  my $color  = Char::GB18030::substr $s, 4, 5;      # black
  my $middle = Char::GB18030::substr $s, 4, -11;    # black cat climbed the
  my $end    = Char::GB18030::substr $s, 14;        # climbed the green tree
  my $tail   = Char::GB18030::substr $s, -4;        # tree
  my $z      = Char::GB18030::substr $s, -4, 2;     # tr

  If Perl version 5.14 or later, you can use the Char::GB18030::substr() function as an
  ...
  end of the string, Char::GB18030::substr() returns the undefined value and produces a
  ...
  my $name = 'fred';
  Char::GB18030::substr($name, 4) = 'dy';         # $name is now 'freddy'
  my $null = Char::GB18030::substr $name, 6, 2;   # returns "" (no warning)
  my $oops = Char::GB18030::substr $name, 7;      # returns undef, with warning
  Char::GB18030::substr($name, 7) = 'gap';        # raises an exception

  An alternative to using Char::GB18030::substr() as an lvalue is to specify the replacement
  ...
  my $s = "The black cat climbed the green tree";
  my $z = Char::GB18030::substr $s, 14, 7, "jumped from";    # climbed
  # $s is now "The black cat jumped from the green tree"

  Note that the lvalue returned by the three-argument version of Char::GB18030::substr() acts
...
=item * Index by GB18030 Character
  ...
  $index = Char::GB18030::index($string,$substring,$offset);
  $index = Char::GB18030::index($string,$substring);
  ...
  $pos = -1;
  while (($pos = Char::GB18030::index($string, $lookfor, $pos)) > -1) {
      print "Found at $pos\n";
      $pos++;
  }
...
=item * Rindex by GB18030 Character
  ...
  $rindex = Char::GB18030::rindex($string,$substring,$offset);
  $rindex = Char::GB18030::rindex($string,$substring);

  This function works just like Char::GB18030::index except that it returns the character
  position of the last occurrence of $substring in $string (a reverse Char::GB18030::index).
  ...
  $pos = Char::GB18030::length($string);
  while (($pos = Char::GB18030::rindex($string, $lookfor, $pos)) >= 0) {
      print "Found at $pos\n";
      $pos--;
  }
...
=item * Filename Globbing
  ...
  This operator is implemented via the Char::Egb18030::glob() function. See Char::Egb18030::glob of
  Char/Egb18030.pm for details.
...
=item * Ordinal Value of Byte
  ...
  of "use Char::GB18030 qw(ord);" exists or not. If $expr is omitted, it uses $_.
  The return value is always unsigned.
...
=item * Reverse List or Byte String
  ...
  "use Char::GB18030 qw(reverse);" exists or not.
...
=head1 Un-Escaping bytes::* Functions (Char/GB18030.pm provides)

Char/GB18030.pm removes 'bytes::' at head of function name.
...
=head1 Escaping Built-in Standard Module (Char/Egb18030.pm provides)

Char/Egb18030.pm does "BEGIN { unshift @INC, '/Perl/site/lib/Char::GB18030' }" at head.
Store the standard module modified for Char::GB18030 software in this directory to
override built-in standard modules.

=head1 Escaping Standard Module Content (You do)

You need copy built-in standard module to /Perl/site/lib/Char::GB18030 and change
'use utf8;' to 'use Char::GB18030;' in its. You need help yourself for now.
...
=head1 Ignore Pragmas and Modules
  ...
  Comment out pragma to ignore utf8 environment, and Char/Egb18030.pm provides these
  functions.
...
=item * Char::GB18030::substr as Lvalue

Char::GB18030::substr differs from CORE::substr, and cannot be used as a lvalue.
To change part of a string, you can use the optional fourth argument which is the
replacement string.

Char::GB18030::substr($string, 13, 4, "JPerl");
...
=item * Special Variables $` and $& need /( Capture All )/
  ...
  Before          After                Works as
  -------------------------------------------------------------------------------------------
  $`              Char::Egb18030::PREMATCH()    CORE::substr($&,0,CORE::length($&)-CORE::length($1))
  $PREMATCH       Char::Egb18030::PREMATCH()    CORE::substr($&,0,CORE::length($&)-CORE::length($1))
  ${^PREMATCH}    Char::Egb18030::PREMATCH()    CORE::substr($&,0,CORE::length($&)-CORE::length($1))
  $&              Char::Egb18030::MATCH()       $1
  $MATCH          Char::Egb18030::MATCH()       $1
  ${^MATCH}       Char::Egb18030::MATCH()       $1
  $'              Char::Egb18030::POSTMATCH()   $'
  $POSTMATCH      Char::Egb18030::POSTMATCH()   $'
  ${^POSTMATCH}   Char::Egb18030::POSTMATCH()   $'
  -------------------------------------------------------------------------------------------
...
Even if ${^WIN32_SLOPPY_STAT} is set to a true value, file test functions Char::Egb18030::*(),
Char::Egb18030::lstat(), and Char::Egb18030::stat() on Microsoft Windows open the file for the path
...
Char::GB18030 software remains one language and one interpreter.
