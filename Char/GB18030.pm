...
sub e_qr {
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # /i modifier
        elsif ($ignorecase and ($char[$i] =~ m/\A [\x00-\xFF] \z/oxms) and (Char::Egb18030::uc($char[$i]) ne Char::Egb18030::lc($char[$i]))) {
            $char[$i] = '[' . Char::Egb18030::uc($char[$i]) . Char::Egb18030::lc($char[$i]) . ']';
        }
...
sub e_require {
    ...
    return qq<Char::Egb18030::require '$expr';>;
}
...
http://search.cpan.org/dist/Char-Char::GB18030/Char::GB18030/JA.pm
...
Definitions in Char/Egb18030.pm.
  ...
  @{Char::Egb18030::eV}             qr{(?:[\x81-\xFE][\x30-\x39][\x81-\xFE][\x30-\x39]|[\x81-\xFE][\x00-\xFF]|[^\x81-\xFE\x0C\x0A\x0D])}
...
=item * Length By GB18030 Character
  ...
  $string is omitted, it returns the Char::GB18030::length of $_.
...
=item * Substr By GB18030 Character
  ...
  area past the end of the string, Char::GB18030::substr raises an exception.
...
=item * Rindex By GB18030 Character
  ...
  $rindex = Char::GB18030::rindex($string,$substring,$position);
  $rindex = Char::GB18030::rindex($string,$substring);

  This function works just like Char::GB18030::index except that it returns the position of
